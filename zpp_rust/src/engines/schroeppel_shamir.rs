...
impl SchroeppelShamirEngine {
    fn run_u128(&self, sh: &Shared) {
        ...
        let q = n / 4;
        let qa: Vec<u128> = nums[0..q].to_vec();
        let qb: Vec<u128> = nums[q..(2 * q)].to_vec();
        let qc: Vec<u128> = nums[(2 * q)..(3 * q)].to_vec();
        let qd: Vec<u128> = nums[(3 * q)..].to_vec();
        ...
        let a_sums = build_sums_u128(&qa, target);
        if sh.stopped() {
            return;
        }
        let b_sums = build_sums_u128(&qb, target);
        if sh.stopped() {
            return;
        }
        let c_sums = build_sums_u128(&qc, target);
        if sh.stopped() {
            return;
        }
        let d_sums = build_sums_u128(&qd, target);
        if sh.stopped() {
            return;
        }
        ...
        while !min_heap.is_empty() && !max_heap.is_empty() {
            ...
            if total == target {
                ...
                if ci_us > 0 {
                    ...
                    if ai_us + 1 < a_sums.len() {
                        let new_sum =
                            a_sums[ai_us + 1].0.saturating_add(b_sums[bi as usize].0);
                        if new_sum <= target {
                            min_heap.push(Reverse((new_sum, (ai_us + 1) as u32, bi)));
                        }
                    }
    ...
    fn run_biguint(&self, sh: &Shared) {
        ...
        let q = n / 4;
        let qa: Vec<BigUint> = p.numbers[0..q].to_vec();
        let qb: Vec<BigUint> = p.numbers[q..(2 * q)].to_vec();
        let qc: Vec<BigUint> = p.numbers[(2 * q)..(3 * q)].to_vec();
        let qd: Vec<BigUint> = p.numbers[(3 * q)..].to_vec();
