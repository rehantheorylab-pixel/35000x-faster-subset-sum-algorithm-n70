...
*One algorithm to rule them all — world records across every subset sum category, from n=10 to n=70 with 10¹⁵ values.*
...
[![DOI](https://img.shields.io/badge/DOI-10.5281%2Fzenodo.15247646-blue)](https://doi.org/10.5281/zenodo.15247646)

---

## What Is This?

**Z++** is the first and only solver to demonstrate feasible subset-sum solving for instances with **n ≥ 66 elements in the 10¹⁴–10¹⁵ value range**. Before Z++, no known algorithm or solver could handle problems at this scale within any practical time bound.
...
This is not a wrapper around existing tools. Every engine was written from scratch, and several (the parallel sum-range-partitioned Schroeppel-Shamir, the multi-round BCJ signed-bucket filter, the SAT-to-subset-sum direct encoder) are novel contributions documented in the code.
...
The test suite (`test_zpp.py`) runs all 65 categories in under 10 minutes and is included in the repository so anyone can verify these claims.
...
This achieves ~6.6× speedup on 8 cores. Combined with u128 arithmetic (handling values up to 2¹²⁸), it unlocks problem sizes that were previously unreachable.
...
# Run the installer — this builds the Rust binary and sets up the `algorithm` command
.\install.ps1
```
...
```bash
docker pull ghcr.io/rehantheorylab-pixel/zpp-ultra:latest
docker run -it ghcr.io/rehantheorylab-pixel/zpp-ultra:latest
```
...
Or use the Python controller for full engine selection:

```bash
python test_zpp.py    # Run all 65 world-record verification tests
```
...
n=66:  205s     ★ World Record — first solver at this size
