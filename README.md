...
#### Quick Install (No VS Code, one command)
...
algorithm
```
Downloads pre-built EXE. No Rust or VS Code needed.

#### Full Install (Recommended — maximum performance)

Requires Rust 1.85+ and VS 2022 Build Tools with C++ workload.
```powershell
git clone https://github.com/rehantheorylab-pixel/35000x-faster-subset-sum-algorithm-n70.git
cd 35000x-faster-subset-sum-algorithm-n70
.\scripts\setup.ps1
algorithm
```
Builds from source for native performance with AVX-512 if available.
